...
def parse_args():
    ...
    parser.add_argument('--eval_max_batches_step', type=int, default=None,
                       help='Max validation batches during step eval (None = use all)')
    parser.add_argument('--eval_max_batches_epoch', type=int, default=None,
                       help='Max validation batches at end-of-epoch (None = use all)')
    ...
    # (eval_every is defined below in Logging and saving)
...
def load_model(model_path, config):
    ...
    # Unwrap different checkpoint layouts
    # - Our pretraining saves {"model_state_dict": ..., "config": ..., ...}
    # - Some tools save {"state_dict": ...}
    ...
    # Check vocabulary size compatibility (robust to different key prefixes)
    def _infer_vocab_size_from_state_dict(sd):
        # Prefer embedding token weight if present
        for k, v in sd.items():
            if k.endswith('embedding.token_embeddings.weight'):
                return v.shape[0]
        # Fallback: output head often has shape [vocab, emb_dim]
        for k, v in sd.items():
            if k.endswith('out_head.weight'):
                return v.shape[0]
        raise KeyError("Could not infer vocab size from state_dict (no embedding or out_head weights found).")

    original_vocab_size = _infer_vocab_size_from_state_dict(state_dict)
...
def create_dataloaders(args, tokenizer):
    ...
    try:
        tlen = len(train_loader.dataset)  # may not exist for some HF iterables
    except Exception:
        tlen = 'unknown'
    try:
        vlen = len(val_loader.dataset)
    except Exception:
        vlen = 'unknown'
    print(f"✅ Train samples: {tlen} | Val samples: {vlen}")
    ...
    for epoch in trange(args.max_epochs, desc="Epoch"):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}")):
            ...
            with autocast(device_type=device_type, dtype=amp_dtype, enabled=(device_type != 'cpu')):
                ...
                loss = loss_fn(logits_shifted.view(-1, logits_shifted.size(-1)),
            ...
            if (batch_idx + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.eval_every and (global_step % args.eval_every == 0) and (val_loader is not None):
                    vloss = sft.evaluate_validation_loss(model, val_loader, loss_fn, device)
                    val_losses.append(vloss)
                    wandb.log({"val/loss_step": vloss, "global_step": global_step})

                if args.save_every and (global_step % args.save_every == 0) and (global_step != last_save_step):
                    save_path = os.path.join(args.output_dir, f"sft_model_step_{global_step}.pt")
                    torch.save(model.state_dict(), save_path)
                    last_save_step = global_step

        # End of epoch: full validation and save checkpoint
        if val_loader is not None:
            vloss_epoch = sft.evaluate_validation_loss(model, val_loader, loss_fn, device)
            val_losses.append(vloss_epoch)
            wandb.log({"val/loss_epoch": vloss_epoch, "epoch": epoch + 1, "global_step": global_step})

        epoch_mean = float(np.mean(epoch_losses)) if len(epoch_losses) > 0 else float('nan')
        train_losses.append(epoch_mean)
        print(f"Epoch {epoch+1}: train_loss={epoch_mean:.4f}")

        ckpt_path = os.path.join(args.output_dir, f"sft_model_epoch_{epoch+1}.pt")
        torch.save(model.state_dict(), ckpt_path)

    print("Saving final model...")
    final_path = os.path.join(args.output_dir, "sft_model_final.pt")
